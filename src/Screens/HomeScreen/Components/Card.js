...
const Card = ({character}) => {
    ...
    const path= '/Card/' + character.id 

    return <div className='tc bg-light-green dib b3 pa3 ma2 grow bw2 shadow-5 br4 mw5'>
        <div >
            <Link to = {path}  > <img src={character.image} alt='characters'/> </Link>
        </div>
        
        <div className='mv0'>
            <Link className='f4 fw6 db dark-green no-underline underline-hover mv0 pa0 h3 ' to = { path }> <h5 className='pv3 mv0'>{character.name}</h5> </Link>
